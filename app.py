...
def preprocess_image():
    try:
        # Leer la imagen desde form-data o cuerpo raw
        ...
        # Preprocesar para modelo (resize + normalizar)
        original_array = np.array(image)
        resized_image = image.resize((256, 256))
        image_array = np.array(resized_image) / 255.0
        image_array = np.expand_dims(image_array, axis=0)

        # Llamar al modelo de segmentación externo
        ...
        # Obtener máscara y escalar a tamaño original
        prediction = np.array(response.json()["predictions"][0])
        mask = (prediction > 0.5).astype(np.uint8) * 255
        mask = mask.squeeze().astype(np.uint8)
        mask_resized = cv2.resize(mask, (original_array.shape[1], original_array.shape[0]))

        # Crear overlay con máscara coloreada sobre imagen original
        ...
        # Responder con imagen PNG binaria
        return Response(overlay_bytes, mimetype='image/png')
